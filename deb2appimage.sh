...
function getappdeps() {
read -n 1 -p "Download the dependency (libraries) manually using the browser from packages.ubuntu.com and copy them to $HOME/.cache/deb2appimage/debs" answer
...
read -n 1 -p "Copy any missing libraries (.so files) to $HOME/.cache/deb2appimage/AppDir/usr/lib/x86_64-linux-gnu you can search for the library from archlinux.org using google advanced search" answer
